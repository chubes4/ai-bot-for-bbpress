...
class AI_HTTP_File_Upload_Client {
    ...
    public static function upload_file($file_path, $upload_url, $headers, $purpose = 'assistants', $timeout = 120) {
        ...
        if ($file_size === false) {
            throw new Exception('Could not determine file size: ' . esc_html($file_path));
        }
        ...
        if ($response === false) {
            throw new Exception('cURL file upload error: ' . esc_html($error));
        }

        if ($http_code >= 400) {
            throw new Exception("HTTP " . esc_html($http_code) . " file upload error: " . esc_html($response));
        }

        $decoded_response = json_decode($response, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new Exception('Invalid JSON response from file upload: ' . esc_html($response));
        }
    ...
    public static function upload_attachment($attachment_id, $upload_url, $headers, $purpose = 'assistants', $timeout = 120) {
        ...
        if (!$file_path) {
            throw new Exception('Attachment not found: ' . esc_html($attachment_id));
        }
    ...
    public static function encode_file_base64($file_path) {
        ...
        if ($content === false) {
            throw new Exception('Could not read file: ' . esc_html($file_path));
        }
